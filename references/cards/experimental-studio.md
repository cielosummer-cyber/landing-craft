...
- 来源：locomotive.ca（2026-08-20 提取，Awwwards 级agency）；验证单：`~/landing-northfield/`（2026-08-20）
...
## 验证修正（2026-08-20，~/landing-northfield/ 首用回填）
...
- 修正 3：验收注意——headless 验证 scramble 时 `page.goto` 默认等 load 事件，Google Fonts 拉取期间动画已放完，采样会误判「动画没跑」；要用 `waitUntil: 'domcontentloaded'` 立即连采（验证脚本留在 `~/landing-northfield/verify-dynamic.mjs`）
